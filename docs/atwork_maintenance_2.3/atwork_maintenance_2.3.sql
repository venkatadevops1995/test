...
-- CHANGES DONE FOR 2.3

-- Employee relieving date staging table.

CREATE TABLE `stage_employee` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `emp_id` int(11) NOT NULL,
  `relieved` date DEFAULT NULL,
  `status` tinyint(2) NOT NULL DEFAULT '1' COMMENT '0-Inactive, 1-Active',
  PRIMARY KEY (`id`),
  KEY `key_stage_employee_status` (`status`),
  KEY `fk_s_e_emp_id_1234_employee_emp_id` (`emp_id`),
  CONSTRAINT `fk_s_e_emp_id_1234_employee_emp_id` FOREIGN KEY (`emp_id`) REFERENCES `employee` (`emp_id`)
  
  
) ENGINE=InnoDB DEFAULT CHARSET=utf8;


alter table leave_balance add column current_leave_balance float(5,2) DEFAULT NULL after comments;
alter table leave_balance add column modified_leave_balance float(5,2) DEFAULT NULL after current_leave_balance;
